...
The real end-user installation path, not just a convenience script — `./install.sh --uninstall` then `./install.sh` is the standard "test as a fresh new user" cycle. It installs system/Flutter build deps, builds the daemon + `tuxdrive-ctl` + `tuxdrive-indicator` + the Flutter GUI, installs everything under `/opt/tuxdrive` and `/usr/local/bin`, writes the systemd user units, and launches the GUI. OAuth credentials resolve in this order: `$TUXDRIVE_CLIENT_ID`/`$TUXDRIVE_CLIENT_SECRET` env vars → `[auth]` section in `config.toml` → the `DEFAULT_CLIENT_ID`/`DEFAULT_CLIENT_SECRET` constants in `packaging/default-credentials.sh` (sourced by both `install.sh` and `packaging/build-packages.sh`, so there's one place to update them) — so a fresh install never has to prompt for credentials. `--uninstall` removes everything under `/opt/tuxdrive`, `~/.local/share/tuxdrive`, `~/.config/tuxdrive`, and the systemd units, but leaves `~/TuxDrive/*` (the synced files) untouched. A from-scratch install needs a C compiler present (`gcc`/`build-essential`, added to `CORE_PKGS` alongside the `-devel`/`-dev` packages) — several crates (`openssl-sys`, `libsqlite3-sys`) compile C shims via build scripts, which fails with `linker \`cc\` not found` on a genuinely fresh machine otherwise.

### `packaging/` — .deb and .rpm packages

The second, parallel install path: `packaging/build-packages.sh [deb|rpm|all]` builds real system packages instead of installing into `/usr/local/bin` + per-user systemd units like `install.sh` does. It hand-rolls the staged file tree with `install`/`cp` (rather than `cargo-deb`/`cargo-generate-rpm`) and wraps it with plain `dpkg-deb --build` / `rpmbuild -bb --buildroot`, so both formats share one staging step and there's no framework-specific asset-path behavior to get wrong. System-package layout differs from `install.sh`'s per-user layout: binaries go to `/usr/bin` (not `/usr/local/bin`), the GUI to `/opt/tuxdrive` (same as `install.sh`), the systemd units to `/usr/lib/systemd/user/` (vendor dir, not `~/.config/systemd/user/`) as **static** files (`packaging/tuxdrive*.service`, fixed `/usr/bin/...` paths — safe because the binary location is fixed at package-build time, unlike `install.sh`'s dynamically-generated units), plus a `.desktop` file and hicolor icon under `/usr/share/`. `packaging/postinst.sh`/`postrm.sh` are shared verbatim between the `.deb` (`DEBIAN/postinst`) and `.rpm` (`%post` scriplet, inlined into the generated spec) — deliberately never call `systemctl --user` there, since a root-context postinst acts on root's own session, not the installing user's; enabling the service is left as a one-line step the user runs themselves (printed in the postinst message). `.github/workflows/release.yml` builds and attaches both formats to a GitHub Release on every `vX.Y.Z` tag push, using `packaging/default-credentials.sh` unless `TUXDRIVE_CLIENT_ID`/`TUXDRIVE_CLIENT_SECRET` repo secrets are set.

### Versioning

`scripts/bump-version.sh X.Y.Z` is the only supported way to bump the version — it keeps `Cargo.toml`'s `[workspace.package].version`, `frontend/flutter/pubspec.yaml`, and `frontend/flutter/lib/version.dart`'s `kAppVersion` in sync (three separate places the version string was duplicated before this script existed). `kAppVersion` also feeds the Flutter GUI's update-checker (`frontend/flutter/lib/services/update_checker.dart`), which compares it against the latest GitHub release tag on startup and shows a dismissible banner if a newer one exists — it never downloads or installs anything itself. Bump the version and add a `CHANGELOG.md` entry for any change a real user would notice (new feature, fixed bug, behavior change) — not for pure internal refactors.
...
| `~/.config/systemd/user/tuxdrive.service` | Systemd user service unit (installed by `install.sh`; `.deb`/`.rpm` ship a static one at `/usr/lib/systemd/user/tuxdrive.service` instead) |
| `/opt/tuxdrive/tuxdrive_flutter` | Installed Flutter GUI binary (same path for both install methods) |
| `/usr/local/bin/{tuxdrive-daemon,tuxdrive-ctl,tuxdrive-indicator}` | Binaries/scripts installed by `install.sh` (`.deb`/`.rpm` install to `/usr/bin/` instead) |
