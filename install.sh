...
# Default OAuth2 credentials baked into this installer so end users never
# need their own Google Cloud project — just login and grant access. A
# Desktop-app client_secret isn't a real secret (Google's own guidance, RFC
# 8252); the actual security boundary is the per-user token in the OS
# keyring, not this value. Override via TUXDRIVE_CLIENT_ID/TUXDRIVE_CLIENT_SECRET
# env vars or an [auth] section in config.toml if you want your own project.
